...
for subdir, dirs, files in os.walk(root_directory):
    ...
    if {'problem_info.json', 'parameters.json', 'optimus-code.py'}.issubset(set(files)):
        # Get the base name of the current directory (e.g., '1', '2', etc.)
        base_name = os.path.basename(subdir)
        
        # Create an output directory within the current directory (e.g., '1_0')
        output_directory = os.path.join(subdir, f'{base_name}_0')
        ...
        for name in sorted_names:
            ...
            code = re.sub(r'\b{}\b'.format(re.escape(name)), replacement, code)
