...
# ---------------------------------
# 1. Helper function to detect LP/MIP from "optimus-code_l.py"
# ---------------------------------
def get_problem_type_from_l(dir_path):
    """
    Reads the 3rd line of optimus-code_l.py (or whichever line you need),
    expecting "# Problem type: LP" or "# Problem type: MIP".
    
    Returns 'LP', 'MIP', or None if it fails to detect.
    """
    e_file = os.path.join(dir_path, "optimus-code_l.py")
...
def main():
    ...
    for dir_name in os.listdir(base_dir):
        ...
        # Figure out if it's LP or MIP by inspecting optimus-code_l.py
        problem_type = get_problem_type_from_l(dir_path)
        if problem_type not in ("LP", "MIP"):
            # No recognized problem type => skip
            print(f"Skipping '{dir_name}' (could not determine LP or MIP from optimus-code_l.py).")
            continue

        # Make sure optimus-code_l.py actually exists (our function already checked, but let's be safe)
        optimus_code_l_path = os.path.join(dir_path, 'optimus-code_l.py')
        if not os.path.exists(optimus_code_l_path):
            print(f"optimus-code_l.py not found in {dir_path}")
            continue

        # Try running optimus-code_l.py
        try:
            result = subprocess.run(
                ['python', optimus_code_l_path],
                capture_output=True,
                text=True,
                check=True
            )
        ...
        solution_l_path = os.path.join(dir_path, 'solution_l.json')

        if os.path.exists(solution_path) and os.path.exists(solution_l_path):
            ...
            with open(solution_l_path, 'r') as f:
                solution_l = json.load(f)
                objective_l = solution_l.get('objective')

            # Check if 'objective' keys exist
            if objective is None or objective_l is None:
                print(f"Objective not found in one of the solution files in {dir_path}")
                continue

            # Compare
            if abs(objective - objective_l) <= tolerance:
