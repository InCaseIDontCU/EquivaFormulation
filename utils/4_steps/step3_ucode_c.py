...
for dir_name in os.listdir(base_dir):
    ...
    if os.path.isdir(dir_path):
        ...
        if os.path.exists(optimus_code_path):
            ...
            updated_code_path = os.path.join(dir_path, 'optimus-code_c.py')

            # Find the subdirectory with '_c' in its name
            subdirs = [d for d in os.listdir(dir_path) if os.path.isdir(os.path.join(dir_path, d)) and '_c' in d]
            if subdirs:
                subdir_name = subdirs[0]  # Assuming only one '_c' subdirectory
                    ...
                    if constraints_start_idx is None or constraints_end_idx is None:
                        ...
                        for idx, line in enumerate(updated_code_lines):
                            ...
                            if 'with open(' in line and 'solution.json' in line:
                                ...
                                # Append '_c' before '.json' in the filename
                                if 'solution.json' in original_path:
                                    new_file_path = original_path.replace('solution.json', 'solution_c.json')
                                else:
                        ...
                        # Write the updated code to optimus-code_c.py
                ...
                print(f"No subdirectory with '_c' found in {dir_path}")
        else:
