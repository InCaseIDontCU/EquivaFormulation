...
# ---------------------------------
# 1. Helper function to detect LP/MIP from "optimus-code_f.py"
# ---------------------------------
def get_problem_type_from_f(dir_path):
    """
    Reads the 3rd line of optimus-code_f.py (or whichever line you need),
    expecting "# Problem type: LP" or "# Problem type: MIP".
    
    Returns 'LP', 'MIP', or None if it fails to detect.
    """
    e_file = os.path.join(dir_path, "optimus-code_f.py")
...
def main():
    ...
    for dir_name in os.listdir(base_dir):
        ...
        # Figure out if it's LP or MIP by inspecting optimus-code_f.py
        problem_type = get_problem_type_from_f(dir_path)
        if problem_type not in ("LP", "MIP"):
            # No recognized problem type => skip
            print(f"Skipping '{dir_name}' (could not determine LP or MIP from optimus-code_f.py).")
            continue

        # Make sure optimus-code_f.py actually exists (our function already checked, but let's be safe)
        optimus_code_f_path = os.path.join(dir_path, 'optimus-code_f.py')
        if not os.path.exists(optimus_code_f_path):
            print(f"optimus-code_f.py not found in {dir_path}")
            continue

        # Try running optimus-code_f.py
        try:
            result = subprocess.run(
                ['python', optimus_code_f_path],
                capture_output=True,
                text=True,
                check=True
            )
        ...
        solution_f_path = os.path.join(dir_path, 'solution_f.json')

        if os.path.exists(solution_path) and os.path.exists(solution_f_path):
            ...
            with open(solution_f_path, 'r') as f:
                solution_f = json.load(f)
                objective_f = solution_f.get('objective')

            # Check if 'objective' keys exist
            if objective is None or objective_f is None:
                print(f"Objective not found in one of the solution files in {dir_path}")
                continue

            # Compare
            if abs(objective - objective_f) <= tolerance:
