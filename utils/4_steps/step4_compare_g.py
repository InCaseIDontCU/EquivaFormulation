...
# ---------------------------------
# 1. Helper function to detect LP/MIP from "optimus-code_2.py"
# ---------------------------------
def get_problem_type_from_2(dir_path):
    """
    Reads the 3rd line of optimus-code_2.py (or whichever line you need),
    expecting "# Problem type: LP" or "# Problem type: MIP".
    
    Returns 'LP', 'MIP', or None if it fails to detect.
    """
    e_file = os.path.join(dir_path, "optimus-code_2.py")
...
def main():
    ...
    for dir_name in os.listdir(base_dir):
        ...
        # Figure out if it's LP or MIP by inspecting optimus-code_2.py
        problem_type = get_problem_type_from_2(dir_path)
        if problem_type not in ("LP", "MIP"):
            # No recognized problem type => skip
            print(f"Skipping '{dir_name}' (could not determine LP or MIP from optimus-code_2.py).")
            continue

        # Make sure optimus-code_2.py actually exists (our function already checked, but let's be safe)
        optimus_code_2_path = os.path.join(dir_path, 'optimus-code_2.py')
        if not os.path.exists(optimus_code_2_path):
            print(f"optimus-code_2.py not found in {dir_path}")
            continue

        # Try running optimus-code_2.py
        try:
            result = subprocess.run(
                ['python', optimus_code_2_path],
                capture_output=True,
                text=True,
                check=True
            )
        ...
        solution_2_path = os.path.join(dir_path, 'solution_2.json')

        if os.path.exists(solution_path) and os.path.exists(solution_2_path):
            ...
            with open(solution_2_path, 'r') as f:
                solution_2 = json.load(f)
                objective_2 = solution_2.get('objective')

            # Check if 'objective' keys exist
            if objective is None or objective_2 is None:
                print(f"Objective not found in one of the solution files in {dir_path}")
                continue

            # Compare
            if abs(objective - objective_2) <= tolerance:
